...
def test(data_test, device, model, loss_fn, set_name, args):
...
def save_result(set, set_name, device, model, args):
    ...
    set: data set (e.g., validation set)
    set_name: name of the data set (e.g., 'validation')
    """
    y_ls = []
    y_pred_ls = []
    des_ls = []
    ...
    isExist = os.path.exists(args.result.path_des)
    if not isExist:
        os.makedirs(args.result.path_des)
    for folder in ['model/', 'des/', 'fig/']:
        path = args.result.path_des + folder
    ...
    torch.save(model.state_dict(), args.result.path_des+'model/model' + args.result.name+set_name + '.pt')
